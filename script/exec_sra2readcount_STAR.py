...
if is_sra == 'yes':
    ...
    print( msg )
    if is_paired=='yes':
        cmd = '%s --sra-id %s/%s.sra --threads %d --split-files --gzip' % (fastq_dump,input_dir,samplename,int(thread_num))
        print( cmd )
        os.system(cmd)
        srcf1 = os.path.join(ws_dir,samplename+'_1.fastq.gz')
        destf1 = os.path.join(sample_dir,samplename+'_1.fastq.gz')
        shutil.move(srcf1,destf1)
        srcf2 = os.path.join(ws_dir,samplename+'_2.fastq.gz')
        destf2 = os.path.join(sample_dir,samplename+'_2.fastq.gz')
        shutil.move(srcf2,destf2)
    else:
        cmd = '%s --sra-id %s/%s.sra --threads %d --gzip' % (fastq_dump,input_dir,samplename,int(thread_num))
        print( cmd )
        os.system(cmd)
        srcf = os.path.join(ws_dir,samplename+'.fastq.gz')
        destf = os.path.join(sample_dir,samplename+'.fastq.gz')
...
if is_qualitycheck == 'yes':
    ...
    print( msg )
    thread_num = str( min(16,int(thread_num)) ) # fastp uses up to 16 threads
    report_html = os.path.join(sample_dir,samplename+'.html')
    report_json = os.path.join(sample_dir,samplename+'.json')
    if is_paired =='yes':
        ...
        cmd = '%s -w %d -h %s -j %s -i %s -I %s -o %s -O %s' % (fastp,int(thread_num),report_html,report_json,fwd_fastq,rev_fastq,trim_fwd_fastq,trim_rev_fastq)
        os.system(cmd)
    else:
        fastq = os.path.join(sample_dir,samplename+'.fastq.gz')
        trim_fastq = os.path.join(sample_dir,samplename+'_trim.fastq.gz')
        cmd = '%s -w %d -h %s -j %s -i %s -o %s' % (fastp,int(thread_num),report_html,report_json,fastq,trim_fastq)
        print( cmd )
        os.system(cmd)
